...
function! MakeStatusLine()
  ...
  if g:config.vimrc.statusline_manually == g:true
    return line
  else
    return ''
  endif
...
function! MakeBigStatusLine()
  if g:config.vimrc.statusline_manually == g:true
    ...
    if filereadable(expand('%'))
      set statusline+=%{rc#misc#get_buf_name(bufnr('%'),'s')}
    else
      set statusline+=%F
    endif
    set statusline+=\ %m
...
function! s:tabpage_label(n)
  ...
  let sp = (no . mod) ==# '' ? '' : ' '
  let fname = rc#misc#get_buf_name(curbufnr, 's')
...
function! GuiTabLabel()
  ...
  "let label .= name
  let label .= rc#misc#get_buf_name('%', 't')
